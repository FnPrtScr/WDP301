      ...
      < div className='d-flex align-items-centerm mx-50'>
        <Col span={3} className="text-center">
          {isFinished ? (
            ...
            <Countdown title="Deadline for iteration 1" value={deadline} onFinish={onFinish} format={`D [days] H [hours] m [minutes] s [seconds]`} />
          )}
        </Col>
      </div>
...
const Position = () => {
  ...
  const initialMilestoneId = dataProjectStudent && dataProjectStudent.length > 0 ? dataProjectStudent[0].milestone_id : null
  const [milestoneId, setMilestoneId] = useState(initialMilestoneId)
  useEffect(() => {
    const obj = dataProjectStudent && dataProjectStudent.length > 0 ? dataProjectStudent[0].milestone_id : null
    setMilestoneId(obj)
  }, [dataProjectStudent])
  ...
  return (
    <Fragment >
      <Card className='overflow-hidden'>
        ...
        <h2 style={{ fontWeight: '700' }} className='px-2 mt-2'>{t('Iteration 1 Information')}</h2>
