...
const Position = () => {
  ...
  const initialMilestoneId = dataProjectStudent && dataProjectStudent.length > 0 ? dataProjectStudent[2].milestone_id : null
  const [milestoneId, setMilestoneId] = useState(initialMilestoneId)
  useEffect(() => {
    const obj = dataProjectStudent && dataProjectStudent.length > 0 ? dataProjectStudent[2].milestone_id : null
    setMilestoneId(obj)
  }, [dataProjectStudent])
  ...
  return (
    <Fragment >
      <Card className='overflow-hidden'>
        ...
        <h2 style={{ fontWeight: '700' }} className='px-2 mt-2'>{t('iteration 3 Information')}</h2>
