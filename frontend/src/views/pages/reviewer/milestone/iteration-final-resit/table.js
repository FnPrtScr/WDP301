...
const App = () => {
  ...
  const optionClass = dataResit.map(item => {
  ...
  const getTeam = (class_id) => {
    const item = dataResit.find(item => item.getClass.class_id === class_id)
    // Map over the arrTeam array of the found item to extract team data
    setOptionTeam(item.arrTeam.map(team => ({
      value: team.te.team_id,
      label: team.te.name
    })))
  }
  const getTeamMember = (team_id) => {
    const item = dataResit.find(item => item.arrTeam.map(e => e.te.team_id === team_id))
    setDataTeamMember(item.arrTeam[0].allHashesCo.map(e => ({
      value: e.user_id,
  ...
  const fetchData = (classId, teamId) => {
    setLoading(true)
    api.finalEvaluationApi.getFinalGradedResitApi({}, campus, semester, milestoneId, classId, teamId)
      .then((rs) => {
  ...
  return (
    <Fragment >
      <Card className='overflow-hidden' >
        <h2 style={{ fontWeight: '700' }} className='px-2 mt-2'>{t('Final Resit Project Presentation Evaluation')}</h2>
              ...
              <Col className='mb-1' md='12' sm='12'>
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <Select
                      id='class_id'
                      value={optionClass.find((val) => val.value === value)}
                      onChange={(e) => {
                        onChange(e.value)
                        getTeam(e.value)
              ...
              <Col className='mb-1' md='12' sm='12'>
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <Select
                      id='team_id'
                      value={optionTeam.find((val) => val.value === value) ?? ''}
                      onChange={(e) => {
                        setDataTeamMember([])
