...
const ModalImportComponent = () => {
  ...
  const renderTableHead = () => {
    if (headArr.length) {
      return headArr[0].map((head, index) => {
        return <th key={index} style={{ width: index === 0 ? '100px' : 'auto' }}>{head}</th>
      })
    } else {
      return null
    }
  ...
  const handleFormOpened = () => {
    setLoading(true)
    api.importHistoryApi.getAllImportHisLecturerApi({}, campus, semester)
      .then((rs) => {
        setDataHis(rs.data)
        setLoading(false)
  ...
  const onSubmit = () => {
    ...
    if (fileImport) {
      ...
      api.userRoleSemesterApi.importLectureApi(formData, campus, semester).then((rs) => {
        if (rs.success === true) {
          handleLoadTable()
          handleModal()
          notificationSuccess(t(rs.message))
          socket.emit('notifications')
        } else {
          handleLoadTable()
          handleModal()
          notificationError(t(rs.message))
          socket.emit('notifications')
        }
      }).catch(() => {
        notificationError(t('Create bulk user failed!'))
      }
      )
    } else {
  ...
  const downloadTemplate = () => {
    const url = `${process.env.PUBLIC_URL}/template/[HEAD]-SPRING2024-LECTURE.xlsx`
    const link = document.createElement('a')
    link.href = url
    link.setAttribute('download', '[HEAD]-SPRING2024-LECTURE.xlsx')
    link.target = '_blank'
    document.body.appendChild(link)
    link.click()
    ...
    <Fragment >
      <Modal
        ...
        <Form onSubmit={handleSubmit(onSubmit)}>
          <ModalHeader typeModal={typeModal} handleModal={handleCancel} title='Import Lecture' />
          <ModalBody>
