  ...
  const getGradeDoc = () => {
    api.teamEvaluationApi.getGradeTeamApi({}, campus, semester, dataItem.iteration_id, dataItem.class_Id, dataItem.team_Id).then((rs) => {
      if (rs.statusCode === 200) {
        ...
        setValue('grade_SRS', data.grade_SRS)
        setValue('grade_SDS', data.grade_SDS)
    ...
    <Fragment >
      <Modal
        ...
        <Form onSubmit={handleSubmit(onSubmit)}>
          <ModalHeader typeModal={typeModal} handleModal={handleCancel} title={typeModal === 'GradeDocument' ? 'Grade Document for iteration1' : 'Edit Setting for iteration1'} />
          <ModalBody>
