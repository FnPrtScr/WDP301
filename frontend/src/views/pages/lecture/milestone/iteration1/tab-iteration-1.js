...
const tabIteration1 = () => {
  ...
  const initialMilestoneId = dataProject && dataProject.length > 0 ? dataProject[0].navLink : null
  ...
  useEffect(() => {
    const obj = dataProject && dataProject.length > 0 ? dataProject[0].navLink : null
    setMilestoneId(obj)
  }, [dataProject])
    ...
    api.iterationApi.getDeadlineApi({}, campus, semester, milestoneId, classId)
      ...
      .catch((e) => {
        if (e.response.data.statusCode === 404) {
          notificationError(t('You have not set a deadline for iteration 1'))
        } else {
