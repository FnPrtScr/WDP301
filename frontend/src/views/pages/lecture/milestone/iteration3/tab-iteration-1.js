      ...
      < div className='d-flex align-items-centerm mx-50'>
        <Col span={3} className="text-center">
          {isFinished ? (
            ...
            <Countdown title="Deadline for iteration 3" value={deadline} onFinish={onFinish} format={`D [days] H [hours] m [minutes] s [seconds]`} />
          )}
        </Col>
      </div >
...
const tabIteration1 = () => {
  ...
  const initialMilestoneId = dataProject && dataProject.length > 0 ? dataProject[2].navLink : null
  ...
  useEffect(() => {
    const obj = dataProject && dataProject.length > 0 ? dataProject[2].navLink : null
    setMilestoneId(obj)
  }, [dataProject])
    ...
    api.iterationApi.getDeadlineApi({}, campus, semester, milestoneId, classId)
      ...
      .catch((e) => {
        if (e.response.data.statusCode === 404) {
          notificationError(t('You have not set a deadline for iteration 3'))
        } else {
  ...
  const fetchDataStatistic = (classId, teamId) => {
    if (classId !== null && teamId !== null) {
      ...
      api.statisticApi.getStatisticLinkProjectTrackingByTeam({}, campus, semester, classId, dataDeadline.iteration_id, teamId)
        .then((rs) => {
          if (rs.statusCode === 200) {
            setDataProjectTracking(rs.data[0]["iteration 3"])
            //} else if (rs.statusCode === 404) {
  ...
  const handleCheckCondition = () => {
    setLoading(true)
    api.LocEvaluationApi.checkConditionPresentApi({}, campus, semester, dataDeadline.iteration_id, classId, teamId)
      .then(() => {
        api.LocEvaluationApi.getTotalLOCApi({}, campus, semester, dataDeadline.iteration_id, classId, teamId)
          .then((rs) => {
            setDataTableTotalLocEvaluation(rs.data[0].map((item, index) => ({
    ...
    },
    {
      ...
      render: (text, record) => (
        <Tag color={record.status ? 'green' : 'red'}>
          {record.status ? t('Passed') : t('Failed')}
        </Tag>
      )
    }
  ]
        ...
        <>
          <Card className='overflow-hidden'>
            <div className='invoice-list-table-header w-100 me-1 ms-50 my-1 mb-75 d-flex justify-content-between flex-wrap px-1'>
              ...
              <div className='d-flex justify-content-end mx-2'>
                <Button className='add-new-semester mx-50 my-25' color='primary' onClick={() => handleCheckCondition()}>
                  {t('Check Condition Present')}
                </Button>
              </div>
            </div >
