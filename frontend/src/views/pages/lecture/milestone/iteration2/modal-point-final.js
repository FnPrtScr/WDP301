          ...
          if (typeof data.graded_point === 'string') {
            api.pointApi.gradePointManualByStudentApi({ student_id: data.student_id, graded_point: +data.graded_point.trim().replace(/\s/g, ''), note: valueF }, campus, semester, dataItem.iteration_id, dataItem.class_Id, dataItem.team_Id).then((rs) => {
              if (rs.statusCode === 201) {
                handleLoadTable()
                handleModal()
                notificationSuccess(t('Grade for student iteration 2 success'))
                socket.emit('notifications')
              } else {
                notificationError(t('Grade for student iteration 2 fail'))
              }
            }).catch((e) => {
              if (e.response.data.statusCode === 404) {
                notificationError(t('No iteration found'))
              } else {
                notificationError(t('Grade for iteration 2 fail'))
              }
            }
            )
          } else {
    ...
    <Fragment >
      <Modal
        ...
        <Form onSubmit={handleSubmit(onSubmit)}>
          <ModalHeader typeModal={typeModal} handleModal={handleCancel} title={typeModal === 'Point' ? 'Grade for student in iteration 2' : 'Edit Grade for student in iteration 2'} />
          <ModalBody>
